...
@pytest.mark.unit
class TestCommentProcessorFailureReactions:
    """Tests for CONFUSED reaction on processing failure."""

    def test_process_adds_confused_reaction_on_failure(self):
        """Test that CONFUSED reaction is added when processing fails."""
        ticket_client = Mock()
        database = Mock()
        runner = Mock()
        ...
        # Create a comment from allowed user
        user_comment = Comment(
            id="IC_1",
            database_id=1,
            body="Update the research",
            created_at=datetime(2024, 1, 15, 10, 0, 0),
            author="allowed_user",
            is_processed=False,
            is_processing=False,
        ...
        # Make _apply_comment_to_kiln_post raise an exception
        with (
            patch.object(processor, "_get_target_type", return_value="research"),
            patch.object(processor, "_extract_section_content", return_value="content"),
            patch.object(
                processor, "_apply_comment_to_kiln_post", side_effect=Exception("Test error")
            ),
            patch("src.comment_processor.set_issue_context"),
            patch("src.comment_processor.clear_issue_context"),
        ):
        ...
        confused_calls = [c for c in reaction_calls if c[0][1] == "CONFUSED"]
        assert len(confused_calls) == 1
        assert confused_calls[0][0][0] == "IC_1"

    def test_process_no_thumbs_up_on_failure(self):
        """Test that THUMBS_UP reaction is NOT added when processing fails."""
        ticket_client = Mock()
        database = Mock()
        runner = Mock()
        ...
        # Verify no THUMBS_UP reaction was added
        reaction_calls = ticket_client.add_reaction.call_args_list
        thumbs_up_calls = [c for c in reaction_calls if c[0][1] == "THUMBS_UP"]
        assert len(thumbs_up_calls) == 0

    def test_process_has_eyes_and_confused_on_failure(self):
        """Test that both EYES and CONFUSED reactions are present on failure."""
        ticket_client = Mock()
        database = Mock()
        runner = Mock()
        ...
        # Verify both EYES and CONFUSED reactions were added (but not THUMBS_UP)
        reaction_calls = ticket_client.add_reaction.call_args_list
        reaction_types = {c[0][1] for c in reaction_calls}
        assert "EYES" in reaction_types
        assert "CONFUSED" in reaction_types
        assert "THUMBS_UP" not in reaction_types

    def test_process_multiple_comments_all_get_confused_on_failure(self):
        """Test that all comments get CONFUSED reaction when processing fails."""
        ticket_client = Mock()
        database = Mock()
        runner = Mock()
        ...
        ticket_client.get_comments_since.return_value = [comment1, comment2]
        ...
        # Verify both comments got CONFUSED reactions
        reaction_calls = ticket_client.add_reaction.call_args_list
        confused_calls = [c for c in reaction_calls if c[0][1] == "CONFUSED"]
        confused_comment_ids = {c[0][0] for c in confused_calls}
        assert confused_comment_ids == {"IC_1", "IC_2"}
